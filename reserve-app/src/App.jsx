            ...
            <div className="flex flex-col items-center text-center">
              <div
                className={`w-8 h-8 sm:w-10 sm:h-10 rounded-full flex items-center justify-center transition-all duration-300 ${
                  index + 1 <= currentStep ? 'bg-red-600 text-white' : 'bg-gray-200 text-gray-500'
                }`}
              >
                {currentStep > index + 1 ? <CheckCircle size={20}/> : index + 1}
              </div>
              <p className={`mt-2 text-xs sm:text-sm transition-all duration-300 ${
                  index + 1 <= currentStep ? 'text-gray-800 font-semibold' : 'text-gray-500'
                }`}>{step}</p>
            </div>
            {index < steps.length - 1 && (
              <div className={`flex-1 h-1 mx-2 transition-all duration-300 ${index + 1 < currentStep ? 'bg-red-600' : 'bg-gray-200'}`}></div>
            )}
          </React.Fragment>
        ))}
      </div>
...
const Step3_InputUserInfo = ({ onNext, onBack, reservation, setReservation, showToast }) => {
  ...
  const validatePhone = (phone) => {
    return /^0\d{1,4}-\d{1,4}-\d{3,4}$|^0\d{9,10}$/.test(phone.replace(/[-－]/g, ''));
  };

  const handleNext = () => {
    if (!formData.name || !formData.phone || !formData.email || !formData.carName) {
      showToast('必須項目（お名前、電話番号、メールアドレス、車名）を入力してください。', 'error');
      return;
    }
  ...
  const InputField = ({ icon, name, label, placeholder, value, onChange, required = false }) => (
    <div>
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-10 pr-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-red-500 focus:border-red-500"
        />
      </div>
    </div>
  );
      ...
      <div className="bg-white p-6 sm:p-8 rounded-lg shadow-lg border border-gray-200 space-y-6">
        ...
        <fieldset className="border-t border-gray-200 pt-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <InputField icon={<User size={18} className="text-gray-400"/>} name="name" label="お名前" placeholder="山田 太郎" value={formData.name} onChange={handleChange} required />
            <InputField icon={<Phone size={18} className="text-gray-400"/>} name="phone" label="電話番号" placeholder="090-1234-5678" value={formData.phone} onChange={handleChange} required />
            <div className="md:col-span-2">
              <InputField icon={<Mail size={18} className="text-gray-400"/>} name="email" label="メールアドレス" placeholder="example@email.com" value={formData.email} onChange={handleChange} required />
            </div>
          </div>
        </fieldset>
        ...
        <fieldset className="border-t border-gray-200 pt-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <InputField icon={<Car size={18} className="text-gray-400"/>} name="carName" label="車名" placeholder="プリウス" value={formData.carName} onChange={handleChange} required />
...
// メインアプリケーション
const App = () => {
  const [step, setStep] = useState(0); // 0: トップページ
      ...
      <footer className="bg-gray-800 text-white mt-12">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="text-center">
            <p>&copy; 2024 Netz Toyota Kitakyushu. All Rights Reserved.</p>
